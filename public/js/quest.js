...
$(document).ready(function() {
    tinymce.init({
        ...
        menubar: false,
        toolbar: "undo redo | p bold italic underline strikethrough superscript subscript | alignjustify alignleft aligncenter alignright | bullist numlist",
        setup : function(ed)
        {
            ed.on('init', function()
            {
...
$(document).on('click', '#add_quest_variable', function ()
{
    ...
    var $questVariable = $('.quest_variable');
...
{
    $(this).parents('.quest_variable').remove();
});

$(document).ready(function(){
    $(document.body).on('click', '.edit_quest_variable' ,function(){
        var button = $(this);
        var variable_row = button.parents('.variable_row');
        $.get("/variable/renderVariableEditForm", {variable_id: button.data('variable_id')}, function (data)
        {
            variable_row.replaceWith(data);
        });
